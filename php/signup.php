...
if(!empty($fname) && !empty($lname) && !empty($nic) && !empty($email) && !empty($password)){
...
if(mysqli_num_rows($sql) > 0){
       ...
       echo "This email alredy exist!";
            ...
            $sql2 = mysqli_query($conn, "INSERT INTO newchat (unique_id, fname, lname, nic, email, password, status)
                                VALUES({$random_id}, '{$fname}',  '{$lname}', '{$nic}' , '{$email}', '{$password}','{$status}')");
