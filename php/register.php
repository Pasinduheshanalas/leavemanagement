...
$epsno = mysqli_real_escape_string($conn,$_POST['epsno']);
$nicno = mysqli_real_escape_string($conn,$_POST['epsno']);
$position = mysqli_real_escape_string($conn,$_POST['position']);
$department = mysqli_real_escape_string($conn,$_POST['department']);



if(!empty($fname) && !empty($lname) && !empty($email) && !empty($epsno) && !empty($nicno) && !empty($position) && !empty($department)){
...
if(mysqli_num_rows($sql) > 0){
    ...
    if(isset($_FILES['image'])){
        ...
        $extensions = ['png','jpeg','jpg'];//these are some valid image ext store array
        if(in_array($img_ext, $extensions) === true){
           ...
           //onece user signedup then his status willbe active now
            ...
            $sql2 = mysqli_query($conn, "INSERT INTO register (unique_id, fname, lname, email,  epsno, nicno, position, department, img)
                                VALUES({$random_id}, '{$fname}',  '{$lname}',  '{$email}', '{$epsno}', '{$nicno}', '{$position}', '{$department}' , '{$new_img_name}')");
